...
string base64_encryption(const string in)                //加密函数
{
    int i = 0;                            //总的控制变量
...
int main()
{
    ...
    string input0;                                                          //存放输入的字符
    string input1;                                                          //存放二进制序列
    string deciToBin_8bit(int);                                    //十进制到二进制函数原型
    string base64_encryption(const string);                                //加密函数原型
    string base64_decryption(const string);
    cout << "Input:" << endl;
    cin >> input0;                                    //输入的ASCII码字符串
    cout << endl;

    for ( auto &x : input0)
        input1 += deciToBin_8bit(static_cast<int>(x));                       //得到二进制序列
